...
[CustomEditor(typeof(HeathenEngineering.UIX.KeyboardDesigner))]
public class HeathenKeyboardKeyDesignerEditorScript : Editor
{
    HeathenEngineering.UIX.KeyboardDesigner designer;

    public override void OnInspectorGUI()
    {
        designer = target as HeathenEngineering.UIX.KeyboardDesigner;
        ...
        {
            EditorGUILayout.HelpBox("You must provide a KeyboardKey prototype. This is simply a prefb representing your key structure that must include a KeyboardKey component. There are several examples in the SystemsUIX > Framework > On Screen Keyboard > Keyboard Prototypes folder.", MessageType.Warning);
        }
        else if (designer.Container == null)
        {
            EditorGUILayout.HelpBox("You must provide a RectTransform target for the keys to be put in, generally this is simply an Empty GameoBject child of the Keyboard.", MessageType.Warning);
        }
        else
        {
            EditorGUILayout.Space();
    ...
    public static void BuildKeyboardKeys(KeyboardDesigner designer)
    {
        ClearAll(designer);
    ...
    static void ClearAll(KeyboardDesigner designer)
    {
        designer.Keys.Clear();
    ...
    static void GenerateKeys(KeyboardDesigner designer)
    {
        if (designer.Container == null)
            return;
        ...
        for (int i = 0; i < designer.rowKeyCount.Count; i++)
        {
            ...
            for (int ii = 0; ii < designer.rowKeyCount[i]; ii++)
            {
                ...
                var kCom = k.GetComponent<KeyboardKey>();
                kCom.keyboard = designer.GetComponent<Keyboard>();
                k.SetActive(true);
    ...
    static void UpdateKeys(KeyboardDesigner designer)
    {
